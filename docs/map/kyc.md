...
- `[docs:<slug>]` — `developer.shaype.com/docs/<slug>.md`. Slugs read: `flexible-kyc-checks`, `customer-creation-1`, `customer-status-flow`, `sample-requests-responses`, `customers`, `customer-removal`, `webhook-notification`. Also checked and found empty of KYC content: `status-transitions` (fetched OK, 0 KYC hits; PayTo/mandate content only). `page/multi-currency-onboarding-and-account-structure` — could not be fetched (HTTP 404, with or without the `page/` prefix); not checked.
...
- The KYC flow (Shaype-run KYC) end to end — steps (1)–(3) [docs:customer-creation-1]: (1) `createCase` → get `scanCase.id`, `webLink`, `mobileToken`; (2) the end user completes identity verification in the web client (`webLink`) or the mobile SDK (`mobileToken`); (3) client calls Customers `createHayCustomer` with `identityVerificationCaseId = scanCase.id`; (4) platform runs the onboarding checks and either activates the customer automatically ("The customer will become active automatically when the KYC is successful") [docs:customer-creation-1] or refers/rejects [docs:customer-status-flow][docs:flexible-kyc-checks]; (5) client receives `ONBOARDING_PASSED` / `ONBOARDING_FAILED` and `CUSTOMER_STATUS_UPDATED` webhooks [docs:customer-creation-1 sample payloads, listed under "Webhook Events" without being tied to a flow step]; sequencing `[inferred]`. The three `approve*Check` operations are the manual override for a failed stage ("If a customer fails a check they will be referred to an operational colleague" [docs:flexible-kyc-checks]).
...
- Purpose: "Create new identity verification case and first submission" [spec summary]. "Create a case is the first step to onboard a customer on the platform when using Shaype KYC functionality" [docs:customer-creation-1]. Marked "(Required only with KYC)" — clients doing their own KYC never call it [docs:customer-creation-1]. The equation of client-KYC with `skipKyc: true` is [docs:customer-creation-1] ("a single endpoint to create a customer, distinguished by the `skipKyc` field"); contradicted by the spec `skipKyc` description, which restricts it to Shaype-KYC clients' agreed test scenarios [spec] — see §7 item 17. Not deprecated.
...
- `ChangeHayCustomerStatusRequestBody.newStatus` — same six-value enum, but its description ("Allowed Customer status. Possible values:") lists only 5 and omits `BLOCKED` [spec] — whether `BLOCKED` is accepted here is undocumented; the client-side alternative to platform activation ("If you are not using Shaype KYC, you will need to manually update the customer status to `ACTIVE`" [docs:customer-creation-1]).

### Webhook payload `NotificationDto` — KYC-relevant subset [webhook-spec]

Delivered to `POST {clientBase}/api/hay/v0/communications/notification` (Shaype retries on client 401/403/429/5XX, **18 times over up to 48 hours** with exponential backoff [docs:webhook-notification]; the docs require "a simple 200 success response" explicitly only from `/email` and `/sms` — for `/notification` any status outside 401/403/429/5XX stops retries [docs:webhook-notification]; answering 200 is `[inferred]`). `required: ["customerHayId","idempotencyKey","type"]`. A second endpoint `POST /api/hay/v1/communications/notification` (operationId `notifyGenericNotification`, body `NotificationDtoV1`, `required: ["idempotencyKey","type"]`, `type` enum `["BATCH_COMPLETED","PERK_ORDER_UPDATE"]`, no `customerHayId`) exists [webhook-spec]; it carries no KYC or customer-status events — all KYC events go to the v0 endpoint with `NotificationDto`.
...
| `firebaseDeviceToken` | string | "Firebase token of the customer's device" — present in both `ONBOARDING_*` doc samples. Source: `HayCustomer.firebaseToken` (string, "Customer's device firebase token (if a mobile app is available)") [spec] — mapping to the webhook field `[inferred]`. No B2B operation sets `firebaseToken` (`CreateHayCustomerRequestBody` and `UpdateCustomerRequestBody` have no such property; the only token ops are Tokens API exchange/elevate), so the local server must seed it via a test hook or omit `firebaseDeviceToken` (optional in `NotificationDto`) |
...
| `PENDING_APPROVAL` | `REFERRED` | platform, "the customer has failed one or more the of the steps such as provided an invalid ID or flagged as a PEP"; "If a customer fails a check they will be referred to an operational colleague" | [docs:customer-status-flow] target-state description (+ [docs:flexible-kyc-checks] for REFERRED); from-state `PENDING_APPROVAL` `[inferred]` |
| `PENDING_APPROVAL` | `REJECTED` | platform, "the onboarding evaluation has concluded that Shaype cannot open an account for the user as a result of the information provided" | [docs:customer-status-flow] target-state description; from-state `PENDING_APPROVAL` `[inferred]` |
...
- **Webhook delivery** [docs:webhook-notification]: `idempotencyKey` per event; retries 18 times / ≤48 h with exponential backoff on client 401/403/429/5XX; docs require 200 explicitly only from `/email` and `/sms`; for `/notification` any status outside 401/403/429/5XX stops retries [docs:webhook-notification]; answering 200 is `[inferred]`. The `ONBOARDING_PASSED` sample reuses the `customerHayId` as `idempotencyKey`; the others use distinct UUIDs [docs:customer-creation-1] — `[inferred]`: always generate a fresh UUID.
...
- **Customers (`customers.md`)**
  ...
  - Duplicate checks (email / phone / doc type+number / name+DOB) fail `createHayCustomer` ("If a duplicate is found, customer creation will fail." [docs:customer-creation-1]); that this is a synchronous error response rather than an async webhook is `[inferred]`; an async `ONBOARDING_FAILED {state: DUPLICATE_CHECK}` path is `[inferred]` from the enum value alone [webhook-spec] — no docs text says a duplicate produces a webhook.
- **Accounts (`accounts.md`)**
  - Account creation is gated on customer `ACTIVE` [docs:customer-status-flow]; the Customers-API create-account error example "Account cannot be created for customer with id ... as their status is currently BLOCKED" shows the gate is enforced with `422` and a `PERMISSION_DENIED:` message prefix [spec ErrorResponse example].
...
- **Webhooks (`notification-webhooks.json`)** — KYC is the **producer** of `ONBOARDING_PASSED`, `ONBOARDING_FAILED` and (on activation/referral/rejection) `CUSTOMER_STATUS_UPDATED` [docs:customer-creation-1][webhook-spec]. Which `actionOwner` a manual approval produces is unstated (`[inferred]`: `CLIENT` for `approve*Check`, `PLATFORM` for automatic outcomes). `firebaseDeviceToken` on the `ONBOARDING_*` payloads: source `HayCustomer.firebaseToken` [spec], mapping `[inferred]`; no B2B operation writes it, so seed via a test hook or omit (optional in `NotificationDto`).
...
The spec attaches the same five error responses to all four operations and documents **no** condition → code mapping and **no** message text for any of them [spec]. Everything below the first two rows is therefore `[inferred]`, patterned on the three `ErrorResponse` examples in the spec: `status` is always the HTTP code as a string and `details` is always `"Please refer to the API documentation or contact Shaype for more info with the traceId."`; two of three messages carry a `PERMISSION_DENIED:` prefix, one (NPP) is plain prose — the prefix is `[inferred]`, not a rule.
...
| `approve*Check`: customer created with `skipKyc: true` / client-KYC (no stages) | 422 | unstated | `[inferred]`; §7 item 17 |
...
| Related, Customers domain: duplicate customer | failure of `createHayCustomer`, code unstated (synchronous error response `[inferred]`); an async `ONBOARDING_FAILED {state: DUPLICATE_CHECK}` is `[inferred]` from the enum value alone | none | failure: [docs:customer-creation-1] "If a duplicate is found, customer creation will fail."; sync vs async: `[inferred]`; async path: `[inferred]` [webhook-spec enum] |
| Related, Customers API (`POST /v0/customers/{customerHayId}/account`): create account for non-ACTIVE customer | 422 | `PERMISSION_DENIED: Account cannot be created for customer with id <uuid> as their status is currently <STATUS>` | [spec ErrorResponse example] |
...
12. **`DUPLICATE_CHECK` timing** — failure of `createHayCustomer` ("If a duplicate is found, customer creation will fail.") is documented [docs:customer-creation-1]; that this is a synchronous error response rather than an async webhook is `[inferred]`; the async `ONBOARDING_FAILED {state: DUPLICATE_CHECK}` path is `[inferred]` from the enum value alone [webhook-spec]. Decide per KYC mode (e.g. sync when no Shaype-KYC stages run, async otherwise — see item 17 for what "mode" means).
...
17. **`skipKyc` semantics** — docs treat it as the switch between Shaype-KYC and client-KYC onboarding ("a single endpoint to create a customer, distinguished by the `skipKyc` field" [docs:customer-creation-1]); spec prose treats it as a permissioned test bypass for Shaype-KYC clients ("Only applicable to Clients using Shaype KYC solution ... Must only set as 'true' in agreed scenarios (i.e. permission to generate a dummy / test account has been granted)" [spec]). Decide whether the local server distinguishes client-KYC clients by configuration rather than by this flag.
