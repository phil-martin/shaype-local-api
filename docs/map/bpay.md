...
- Validation rules, verbatim [docs:bpay]:
  ...
  - "Throw error if active biller is not found for that biller code" → the biller code is looked up in a biller directory (Staging uses a mock — see §2 Staging biller fixtures; no source names the live directory — Cuscal/the BPAY biller list [inferred]) and must exist and be active.
...
No currency field: the amount is in the account's currency (webhook example shows `AUD`) [inferred from docs:bpay example]. The request does not reference a saved biller id; it carries `billerCode`+`reference` directly [spec], so a saved biller is not a precondition [inferred].
...
4. "The `amount` field cannot be zero and must not exceed the available balance in the account." [docs:bpay] → `REFUSED_INSUFFICIENT_FUNDS` [inferred from enum name; no source maps this rule to that outcome]. `REFUSED_INSUFFICIENT_FUNDS` is in this endpoint's enum [spec]; the outcome catalogue lists `REFUSED_NOT_ENOUGH_FUNDS` for MIN_BALANCE breaches generally [docs:payment-transaction-outcome], which is *not* in this endpoint's enum [spec].
...
6. Biller code must be a known, active biller → [inferred mapping] `REFUSED_BPAY_INVALID_BILLER_CODE` ("invalid BPAY biller code provided" [docs:payment-transaction-outcome]); reference must pass the biller's CRN length/check-digit rules → [inferred mapping] `REFUSED_BPAY_INVALID_REFERENCE` ("invalid BPAY reference number" [docs:payment-transaction-outcome]). Which rules apply per biller are the Staging fixture fields "Valid CRN Lengths", "Check digit rule", "Variable CRN Indicator" [docs:bpay].
...
11. External-balance clients: Shaype calls the client's `POST /transactions` with `authorisationTransactionType: "BPAY_TRANSFER_OUT"` before completing [ext-auth-spec]; client refusal is HTTP 470 with `errorCode` ∈ {`REFUSED_MAX_BALANCE_EXCEEDED`, `REFUSED_NOT_ENOUGH_FUNDS`, `REFUSED_SENDER_ACCOUNT_NOT_VERIFIED`} [ext-auth-spec]. Only `REFUSED_MAX_BALANCE_EXCEEDED` and `REFUSED_NOT_ENOUGH_FUNDS` are supported; any other client response (including `REFUSED_SENDER_ACCOUNT_NOT_VERIFIED`, which the YAML enum lists) defaults to outcome `INTERNAL_ERROR` — "Currently, only the following two error codes are supported, any other response sent by the client will default to an outcome of INTERNAL_ERROR" [docs:external-authorisation-and-balance]. The API `outcome` produced by the two supported codes is not stated; `REFUSED_INSUFFICIENT_FUNDS` for `REFUSED_NOT_ENOUGH_FUNDS` [inferred]. Non-scheme timeout is 10 s [docs:external-authorisation-and-balance]; BPAY falling in the non-scheme bucket is [inferred] (the doc lists "NPP, Direct Entry, General credit/debit, etc."; spec groups `bpay` under `ClientLiquidity.nonScheme`). On timeout the transaction is "automatically rejected with outcome: INTERNAL_ERROR" [docs:external-authorisation-and-balance]. See §5 and §7 item 23.
...
**Behaviour:**
- Looks the biller up by `billerCode` and validates `reference` against that biller's CRN rules (lengths, check digit, variable-CRN indicator, and for `ICRNAMT` billers the embedded amount) [docs:bpay, via the Staging fixture fields]. Success returns `BPayBillerDetails` [spec]; that `referenceNumber` echoes the request `reference` and the other fields come from the biller directory is [inferred].
...
`status` (`ACTIVE`/`DISMISSED`) is writable via PATCH [spec] and is presumably persisted on the record [inferred]; it appears on no response schema [spec]. The webhook-side `billerImage` example value is `"https://images.lookwhoscharging.com/8d9595b6-812e-4e32-8a58-fedbe856b2f2/iinet-ci-image.png"` [docs:bpay] — a URL, so `image` is most plausibly a URL too [inferred].
...
Nothing required. Example values from the Staging fixtures [docs:bpay], with fixture "Industry Code" read as `industryAnzsicCode` [inferred]: `billerCode` `"7773"`, `longName` `"APIBCD SERVICES AV1"`, `industryAnzsicCode` `"1113"` ("1113 - Cured Meat and Smallgoods Manufacturing"). Note fixture `93880` lists industry code `94540` (five digits), contradicting "four-digit".
...
`makeBpayPayment` with `ACCEPTED` presumably produces a transaction retrievable via `getTransactionById` (`GET /v1/transactions/{transactionHayId}`) and `searchTransactions` (`POST /v0/transactions/search`, filters: `accountId`, `fromDateTimeUtc`, `toDateTimeUtc`, `originChannel`, `originId`, `originType`) [inferred from spec enums — the spec never states that `makeBpayPayment` creates a `FinancialTransaction`; the operations and filters themselves are [spec]]. Relevant enum members [spec]:
...
- `FinancialTransaction.reference` description: "Reference on the transaction (only applicable to NPP transactions), maximum 35 alphanumeric characters in length" — so the BPAY CRN may not be in `reference` [spec].
...
| `1016` | 3501 - Car Wholesaling | BILLER LONG NAME 505529 | 10 | N | 42741454 | MOD10V01 | NONE (Inactive Biller) | — | N/A | **no** (Deactivated) |
...
- `longName` ≤ 50 chars, `shortName` ≤ 20 chars, both "Commas are not allowed"; `billerName` (main-spec `BpayDetails`) 1–50 chars ("Name of the BPAY Biller, 1 to 50 characters in length"); webhook `BpayDetails.billerName` has no stated bound (description is just "Biller name.") [webhook-spec]. `industryAnzsicCode` "four-digit numbers".
...
- "Daily limit are calculated on a rolling 24h window. When we receive a transaction request, the limit checker will get all transactions from the past 24h for that account and check if the total (including the current transaction) would go over the limit." [docs:account-limits] — read as a trailing wall-clock 24 h rather than a calendar/business day [inferred]; the account-limits doc's ATM example says "until the next calendar day", which contradicts "rolling 24h window" — treat rolling as authoritative for BPAY since the BPAY doc cross-links to it [inferred].
...
- Alerts are emails, not webhooks, and do not refuse transactions. `LiquidityThreshold`: `type` string enum [`TOTAL_DAILY_INBOUND_DIRECT_DEBIT`, `TOTAL_DAILY_NET_NON_SCHEME`, `TOTAL_DAILY_NET_VISA`, `TOTAL_DAILY_OUTBOUND_BPAY`] (the field that selects this threshold type), `percent` 1–100 int32 nullable, `amount` ≥ 1 nullable, `active`, `percental`, `external`, `clientReference` (minLength 1), `id` (uuid); required: `clientReference`, `id`, `type` [spec].
...
| **Customers** | `senderCustomerHayId` "Unique identifier (UUID) of the Customer (initiator of the transfer)" [spec]; the webhook example carries one value (`63d24ae0-d497-485e-800a-ad141542d23r`) as both `transactionEvent.customerHayId` and top-level `NotificationDto.customerHayId` [docs:bpay]; that these echo the request's `senderCustomerHayId` is [inferred] — the example shows only the webhook, not the originating request | — | [spec][docs:bpay][inferred] |
| **Transactions** (`getTransactionById`, `searchTransactions`, tags) | — | Creates a `FinancialTransaction` with `type: BPAY_TRANSFER_OUT`, `transactionChannel: CUSCAL_BPAY_TRANSFER_OUT`, `originType: CUSTOMER`; returns its id as `transactionId` [inferred from spec enums; the spec never states that `makeBpayPayment` creates such a record or which channel/originType it carries]. `BPAY_IN_REJECT` channel exists (purpose not described; reversals/rejections [inferred from name]). `BPAY_TRANSFER_IN` and `CUSCAL_BPAY_TRANSFER_IN` are "not currently in use" [spec] | [spec: enum values and descriptions][inferred] |
...
| **External authorisation** (Shaype → client, `external-balance.yaml`) | For clients holding balances externally, Shaype calls client `POST /transactions` with `Transaction{transactionId, authorisationTransactionType: "BPAY_TRANSFER_OUT", accountId, amount{amount: string, currency}, counterpartDetails{name, basicAccountNumber}, description, reference, account{id, balance, holder{id, type: CUSTOMER|GROUP}, statistics{txn_count_last_10m, txn_sum_last_24h}}, customer{id, details, address, tenure_days}, transactionTimeUtc}` plus headers `Shaype-Version`, `Shaype-Trace-Id`, `Shaype-Idempotency-Key`, `Shaype-Timestamp`, `Shaype-Signature`, `Shaype-Key-Id`. Client answers 200 (approve) or 470 `Response{errorCode ∈ REFUSED_MAX_BALANCE_EXCEEDED | REFUSED_NOT_ENOUGH_FUNDS | REFUSED_SENDER_ACCOUNT_NOT_VERIFIED, reason}` or 500 [ext-auth-spec]. Only `REFUSED_MAX_BALANCE_EXCEEDED` and `REFUSED_NOT_ENOUGH_FUNDS` are supported; any other client response (including `REFUSED_SENDER_ACCOUNT_NOT_VERIFIED`, which the YAML enum lists) defaults to outcome `INTERNAL_ERROR` [docs:external-authorisation-and-balance]. Non-scheme timeout is 10 s [docs:external-authorisation-and-balance]; BPAY falling in the non-scheme bucket is [inferred] (the doc lists "NPP, Direct Entry, General credit/debit, etc."; spec groups `bpay` under `ClientLiquidity.nonScheme`); on timeout → `INTERNAL_ERROR`. Platform limit checks run first and "reject the transaction without performing a balance check on the client's side"; the limits the doc lists as checked are `ATM_WITHDRAWAL_PER_DAY`, `TOP_UP_PER_DAY`, `CARD_PAYMENTS_DAILY`, `PAYMENT_TO_ACCOUNT_NUMBER`, `SINGLE_CARD_TRANSACTION`, `TOTAL_SPEND_PER_YEAR` [docs:external-authorisation-and-balance] — `BPAY_DAILY_LIMIT` is not in that list | — | [ext-auth-spec][docs:external-authorisation-and-balance] |
...
| `amount` "cannot be zero and must not exceed the available balance" | `makeBpayPayment` | 200 `REFUSED_INSUFFICIENT_FUNDS` [inferred from enum name; value is in the enum [spec]]; zero amount fails schema first (400) [inferred] | [docs:bpay][spec] |
...
| External-balance client refuses | `makeBpayPayment` (via callback) | client HTTP 470 `errorCode` `REFUSED_MAX_BALANCE_EXCEEDED` / `REFUSED_NOT_ENOUGH_FUNDS` / `REFUSED_SENDER_ACCOUNT_NOT_VERIFIED` [ext-auth-spec]. Only `REFUSED_MAX_BALANCE_EXCEEDED` and `REFUSED_NOT_ENOUGH_FUNDS` are supported; any other client response (including `REFUSED_SENDER_ACCOUNT_NOT_VERIFIED`, which the YAML enum lists) defaults to outcome `INTERNAL_ERROR` [docs:external-authorisation-and-balance]. API `outcome` for the two supported codes not stated (`REFUSED_INSUFFICIENT_FUNDS` for `REFUSED_NOT_ENOUGH_FUNDS` [inferred]) | [ext-auth-spec][docs:external-authorisation-and-balance] |
...
18. **Amount-vs-balance vs external balance.** For an external-balance client, does the platform still enforce "must not exceed the available balance" itself, or delegate wholly to the callback? The doc's pre-check list is `ATM_WITHDRAWAL_PER_DAY`, `TOP_UP_PER_DAY`, `CARD_PAYMENTS_DAILY`, `PAYMENT_TO_ACCOUNT_NUMBER`, `SINGLE_CARD_TRANSACTION`, `TOTAL_SPEND_PER_YEAR` [docs:external-authorisation-and-balance]; BPAY_DAILY_LIMIT is not in that list though the BPAY doc says it is checked.
...
23. **External-authorisation `errorCode` enum: YAML vs docs disagree.** `external-balance.yaml` `Response.errorCode` enumerates three values (`REFUSED_MAX_BALANCE_EXCEEDED`, `REFUSED_NOT_ENOUGH_FUNDS`, `REFUSED_SENDER_ACCOUNT_NOT_VERIFIED`) [ext-auth-spec], but the docs say "only the following two error codes are supported" — `REFUSED_MAX_BALANCE_EXCEEDED`, `REFUSED_NOT_ENOUGH_FUNDS` — and "any other response sent by the client will default to an outcome of INTERNAL_ERROR" [docs:external-authorisation-and-balance]. Suggest: a 470 carrying `REFUSED_SENDER_ACCOUNT_NOT_VERIFIED` (or any unlisted code) → `INTERNAL_ERROR`; `REFUSED_NOT_ENOUGH_FUNDS` → `REFUSED_INSUFFICIENT_FUNDS` [inferred]; `REFUSED_MAX_BALANCE_EXCEEDED` has no counterpart in `BpayPaymentResponseBody.outcome` — undecided.
