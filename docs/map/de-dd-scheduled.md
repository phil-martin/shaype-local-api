...
- Behaviour: presumably as v1 (below), with the 4-value v0 outcome vocabulary [inferred — the spec states nothing about v0 behaviour beyond the schemas and the deprecation notice]. Implementer note: a cleanroom server should treat v0 as a thin facade over the v1 record, mapping v1 `RECEIVED` → ? and `COMPLETE`/`INCOMPLETE` → ? — no mapping is documented (section 7) [inferred].
- Webhooks: presumably as v1 [inferred — docs reference only the v1 endpoint].
...
- Query params [spec]:
  ...
  - `senderAccountNumber` — string, optional. "Sender Account Number" [spec]; presumably filters on `DeTransactionDetailsV1.senderAccountNumber` (the account to be credited) [inferred].
- Request body: none [spec].
...
- Webhooks [docs:direct-debits, spec:webhooks]:
  - `type: "DIRECT_ENTRY"` notification with `directEntryEvent: { transactionId, type: "DEBIT", direction: "OUTBOUND", status }` — one per status change; RECEIVED and ACCEPTED synchronously with the request, SUBMITTED and COMPLETE later [docs:direct-debits]. The docs state verbatim only that DIRECT_ENTRY is sent "for each of the statuses in the diagram above" — the diagram is an image not readable as text, so whether RETURNED / REJECTED / INCOMPLETE webhooks are also sent is [inferred] from the `DirectEntryEventDto.status` enum, not stated. "The DIRECT_ENTRY notification is about the request" [docs:direct-debits].
...
- Behaviour:
  - Read-only [inferred].
  - Scope of ids: the webhook DTO says "Currently only the `DEBIT` type is supported" and "Currently only the `OUTBOUND` direction is supported" [spec:webhooks], and the enum is byte-identical to the v1 Direct Debit outcome enum, so in practice the ids accepted here are the outbound-DD `transactionId`s created by `createDirectDebitV1`/`V0` [inferred]. Direct Credits are initiated via the Payments domain (`makeTransferV1` — docs reference slug `/reference/maketransferv1`; `transferType: ACCOUNT`) and are not covered by this lookup as far as the docs state [docs:direct-debits, inferred].
- Webhooks: none [inferred].
...
"Details of the scheduled or recurring payment." Returned by `getScheduledPayments` (array) and `getScheduledPaymentById`; status mutated by `cancelScheduledPayment`; created/updated only via the GraphQL/UI portal mutations `createScheduledPayment` / `updateSchedulePayment` (or by request to Shaype's CSM team) [spec, docs:scheduled-payments]. No field is marked required [spec].
...
Delivered to the client's `POST /api/hay/v0/communications/notification` (operationId `notifyNotification`) as a `NotificationDto` [spec:webhooks]. `NotificationDto` required fields: `customerHayId` (uuid), `idempotencyKey` (uuid, "to uniquely represent this request and prevent duplication"), `type`. Optional envelope fields: `productId` (uuid) and `firebaseDeviceToken` appear in the docs samples (`productId` in the DIRECT_DEBIT_TRANSFER sample, `firebaseDeviceToken` only in the INTERBANK_TRANSFER_OUT sample; the DIRECT_ENTRY sample carries only the three required fields) [docs:direct-debits]; `actionOwner` (`["CLIENT", "PLATFORM"]`) is defined on `NotificationDto` [spec:webhooks] but not shown in any sample for this domain.
...
Terminal states: CANCELLED, DELETED, REPLACED, COMPLETED, FAILED, REJECTED — CANCELLED, COMPLETED, DELETED are described as "no longer active", FAILED and REJECTED as "any further scheduled payments will not be processed" [spec]; REPLACED is described only as "replaced with a newer version", so treating it as terminal is [inferred]; no reactivation path is documented [inferred]. Whether `cancelScheduledPayment` on a non-ACTIVE schedule errors or is a no-op is undocumented (section 7).
...
| RECEIVED | ACCEPTED | platform validation/authorisation passes — "accepted and awaiting submission to next Direct Entry payment batch" [spec description; transition inferred]; DIRECT_ENTRY webhook sent synchronously with the request [docs:direct-debits] |
| RECEIVED | REJECTED | platform — "request failed validation or authorization and can't be executed" [spec description; transition inferred]; terminal |
...
| SUBMITTED | COMPLETE | platform, "After two working days the requested customer account is credited and a notification of the successful transaction is sent" [docs:direct-debits]; sender account balance += amount [docs:direct-debits]; DIRECT_ENTRY (COMPLETE) webhook [docs:direct-debits]; + TRANSACTION (DIRECT_DEBIT_TRANSFER) webhook at COMPLETE [inferred — docs say only that `DIRECT_DEBIT_TRANSFER` "is for the actual transaction"; see 7.9]; terminal |
| SUBMITTED | INCOMPLETE | platform, "request was successful, but customer account crediting failed" [spec description; transition inferred]; terminal as far as documented |
...
- `CurrencyAmount.amount` is a JSON number "to 2 decimal places"; `currency` is ISO 4217 [spec].
...
- **Balances**: DD COMPLETE credits the sender account by `amount` [docs:direct-debits] and presumably emits `TRANSACTION`/`DIRECT_DEBIT_TRANSFER` with `accountBalances` [inferred; see 7.9]. Scheduled occurrences debit the account like any outbound transfer/BPAY [inferred].
- **Transactions**: `FinancialTransaction.originType` enum includes `SCHEDULED_PAYMENT` ("Transaction initiated by a schedule") and `DIRECT_DEBIT` (no description in the B2B spec — its `originType` description covers only CUSTOMER, SCHEDULED_PAYMENT, HAAS_OPERATIONS, OPERATIONS; the webhook spec's `TransactionEventDto.originType` describes it as "Transaction initiated by direct debit" [spec:webhooks]); `FinancialTransaction.originId` = "Additional identifier applied to Transaction related to origin of the request"; `FinancialTransaction.type` includes `DIRECT_DEBIT_TRANSFER` ("Cash transfer out of Account via Direct Debit"), `INTERBANK_TRANSFER_OUT` ("Cash transfer out of Account via Direct Credit or NPP"), `BPAY_TRANSFER_OUT` [spec]. `SearchTransactionsRequestBody.originType` and `CreateTransactionRequestBody.originType` also accept `SCHEDULED_PAYMENT` / `DIRECT_DEBIT` [spec].
...
- **Liquidity (`GET /v1/liquidity`, `getClientLiquidity`)**: `ClientLiquidity` → `NonSchemeLiquidity` → `DirectEntryLiquidity { credit: DirectCreditLiquidity, debit: DirectDebitLiquidity { inbound, outbound, total }, total }` [spec] — outbound DDs created here presumably feed `debit.outbound` [inferred]. The only place the spec models inbound vs outbound DD volumes (bears on 7.10).
- **Utilities (mock DE inbound)**: `POST /v0/utils/generate-de-inbound` (`generateInboundDeTransaction`, tag `Utilities API` — "Set of assorted Utility APIs allowing creation of mock transactions", summary "Generate mock inbound DE request.", body `GenerateInboundDeRequestBody` — "Body of an inbound DE request.", 200 → `GenericMessage`, errors 400/403/422/500/501 → `ErrorResponse`) [spec]. Required fields: `amount` (number, `minimum: 0`, `exclusiveMinimum: true`), `recipientAccountNumber` (pattern `\d{5,9}`), `recipientBsb` (`\d{6}`), `recordType` enum `["DIRECT", "RETURN", "REFUSAL"]` ("DIRECT: Money transfer request; RETURN: Return money transfer request with a reason; REFUSAL: Refuse to accept return of request with a reason"), `senderAccountNumber` (`\d{5,9}`), `senderBsb` (`\d{6}`), `transactionType` enum `["CREDIT", "DEBIT"]`; optional: `description`, `idempotencyKey` (uuid, "to uniquely represent this request and prevent duplication"), `recipientName`, `senderName`, `returnReason` enum `["INVALID_BSB_NUMBER", "PAYMENT_STOPPED", "ACCOUNT_CLOSED", "CUSTOMER_DECEASED", "NO_ACCOUNT_OR_INCORRECT_ACCOUNT_NUMBER", "REFER_TO_CUSTOMER", "INVALID_USER_ID", "TECHNICAL_INVALID"]` ("Required for record type RETURN"), `refusalReason` enum `["RETURN_RECEIVED_OUT_OF_TIME", "INSUFFICIENT_INFORMATION_TO_APPLY", "REVERSAL_OF_DUPLICATED_ITEM", "NO_ARRANGEMENT", "TECHNICALLY_INVALID"]` ("Required for record type REFUSAL") [spec]. This schema is the only place the spec defines DE direction semantics verbatim: `transactionType` "CREDIT: Move money from sender account to recipient account; DEBIT: Move money from recipient account to sender account" [spec] — corroborating the sender=credited / recipient=debited reading under `createDirectDebitV1`. The op's request examples are "Inbound Direct Credit request" (`recordType: DIRECT`, `transactionType: CREDIT`), "Inbound Direct Debit request" (`recordType: DIRECT`, `transactionType: DEBIT`), "Refuse return of inbound direct debit request" (`recordType: REFUSAL`, `refusalReason: RETURN_RECEIVED_OUT_OF_TIME`, `transactionType: DEBIT`) and "Return outbound direct debit request" (`recordType: RETURN`, `returnReason: ACCOUNT_CLOSED`, `transactionType: DEBIT`, with sender/recipient BSB+account swapped relative to the inbound examples) [spec]. The "Return outbound direct debit request" example is the platform-provided way to drive an outbound DD to `RETURNED` [spec example name; the matching rule (by BSB/account/amount? by id?) is not documented — see 7.8]. How a mock inbound request is matched to a Shaype account, what it does to balances/status, and whether it emits webhooks are all undocumented [spec].
- **Payments (Direct Credit)**: Direct Credit is *not* in this domain: it goes through `makeTransferV1` (docs reference slug `/reference/maketransferv1`) with `transferType: ACCOUNT`; the platform routes to INTERNAL (Shaype BSB) → NPP (if enabled) → DE otherwise, and emits `INTERBANK_TRANSFER_IN` / `INTERBANK_TRANSFER_OUT` TRANSACTION webhooks [docs:direct-debits].
...
- **Webhooks**: emits `DIRECT_ENTRY` (per DD status) [docs:direct-debits], `SCHEDULED_PAYMENT` (on creation via GraphQL) [spec:webhooks]; `TRANSACTION` (`DIRECT_DEBIT_TRANSFER`) at DD COMPLETE, and scheduled occurrences as their own transactionType with `originType: SCHEDULED_PAYMENT`, are [inferred] — neither source states the webhook shape for either (see 7.9). Delivery: retried "18 times over a period of up to 48 hours" with exponential backoff on client responses 401, 403, 429, 5XX [docs:webhook-notification].
...
- **PayTo mandates** (separate domain): `GetMandateActionsDetailsCreationDto.mandateType` enum is the single (apparently malformed) value `"DIRECT_DEBIT,STANDING_ORDER"` [spec] — unrelated to BECS direct debits; do not conflate.
...
| `createDirectDebitV1` validation/authorisation failure surfaced as a business outcome | 200 | `DirectDebitResponseV1` with `outcome: "REJECTED"` ("Direct Debit request failed validation or authorization and can't be executed") and `details` | [spec] enum description; HTTP-200 mapping inferred |
...
8. **`processingDate` derivation** (request date? next batch business day?) and the exact "2 working days" clock (calendar used, cut-off time, timezone) for SUBMITTED → COMPLETE/RETURNED in a local simulator. For driving RETURNED the spec already provides `POST /v0/utils/generate-de-inbound` with `recordType: "RETURN"`, `transactionType: "DEBIT"`, `returnReason` (the "Return outbound direct debit request" example, section 5) [spec] — implement that rather than a bespoke hook; how it matches the outbound DD record (BSB + account number + amount? most recent SUBMITTED?) is undocumented [spec]. No spec-provided way to advance ACCEPTED → SUBMITTED or SUBMITTED → COMPLETE/INCOMPLETE exists; a clock/batch trigger for those remains the implementer's choice [inferred].
9. **Which transaction(s) the credit leg of an outbound DD produces.** The docs' `DIRECT_DEBIT_TRANSFER` sample is a *debit* of the customer ("external bank account pull funds from customer account"), and the spec describes `DIRECT_DEBIT_TRANSFER` as "Cash transfer out of Account via Direct Debit", whereas an outbound DD *credits* the sender at COMPLETE. Decide the `transactionType`, sign of `currencyAmount`, `originType` (`DIRECT_DEBIT`?) and `originId` for the credit, and whether a separate transaction is emitted for RETURNED/INCOMPLETE. Note the *inbound* DD the docs sample depicts is modelled in the spec by `POST /v0/utils/generate-de-inbound` with `recordType: "DIRECT"`, `transactionType: "DEBIT"` (section 5) [spec]; its `DEBIT` description "Move money from recipient account to sender account" makes the customer (recipient) the debited party [spec].
10. **Whether `DIRECT_DEBIT_PER_DAY` applies to this flow.** Spec ("outgoing direct debit transfers") and docs ("outgoing cash from inbound direct debit requests") disagree; decide whether outbound-DD creation is limit-checked at all, and against which account. Inbound DDs (which debit the customer) are modelled by `POST /v0/utils/generate-de-inbound` with `recordType: "DIRECT"`, `transactionType: "DEBIT"` (section 5) [spec]; if the limit is applied to inbound DDs, that utility is where the local server would enforce it [inferred].
