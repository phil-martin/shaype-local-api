...
- `[spec]` = `notification-webhooks.json` (OpenAPI "Notification Webhooks" v1.0.0, `x-explorer-enabled: false`, server `http://localhost:8080`, tag `Notifications API` — "Notifications (webhooks) API that should be implemented by client"). Each reference page embeds a per-endpoint subset of the same OpenAPI: `reference/notifynotification` carries only the v0 `/notification` path + 29 schemas, `reference/notifygenericnotification` only the v1 path + its 7 schemas (NotificationDtoV1, EventDetailsDto, BatchCompletedEventDto, ItemStatisticsDto, PerkOrderUpdateEventDto, RedemptionDto, ValidityDto). Every schema present is byte-identical to `notification-webhooks.json` (verified).
...
| `REMINDER` | A reminder | `reminderType` (string enum, 19 values) + for card-expiry reminders `cardExpiryReminderEvent` | `CardExpiryReminderEventDto` | The `APPLE_PAY_ADD_TO_WALLET_REMINDER_*` samples put `cardHayId` on the envelope; the `APPLE_PAY_SPEND_REMINDER_*` samples do not `[docs:apple-and-google-pay-notifications]` |
...
`type` enum verbatim (`jq '.components.schemas.NotificationDtoV1.properties.type.enum'`): `["BATCH_COMPLETED","PERK_ORDER_UPDATE"]`. Note the `type` description text only lists BATCH_COMPLETED; the enum has both. `eventDetails` is `oneOf [BatchCompletedEventDto, PerkOrderUpdateEventDto]`, both `allOf`-extending `EventDetailsDto`, whose `eventType` (same 2 values) is the OpenAPI `discriminator.propertyName`. So a v1 payload *may* carry the event kind twice — `type` (required) on the envelope and the optional `eventDetails.eventType` discriminator inside `[spec]`; whether both are always populated is not stated `[inferred]`.
...
Included for completeness; these carry auth-service / Accelerator-app events. Their `type` enums are **different** from `NotificationDto.type` and are not part of the v0 `/notification` business stream (`NotificationDto.type`, §2.1).
...
- No documented timeout for the client's response to *webhooks* `[inferred: none]`. (The *external authorisation* API, a different contract, enforces 1.2 s (1200 ms) for scheme/card transactions and 10 s for non-scheme transactions, round-trip included; on timeout the transaction is rejected with outcome `INTERNAL_ERROR` `[docs:external-authorisation-and-balance]`.)
...
| `INTRABANK_TRANSFER_IN` | `false` | op:`makeTransferV1` (and deprecated `makeTransferV0`) with `transferType` `ACCOUNT` to a Shaype BSB ("automatically converted to an INTERNAL (ShaypePay) transaction") — delivered to the **recipient** customer | after execution; "You will receive this event for internal account transfers (Shaype to Shaype). This event is for the recipient of the funds." [docs:payments] | [docs:payments] [docs:direct-debits] (conversion quote) |
| `INTRABANK_TRANSFER_OUT` | `false` | same as above — delivered to the **sender** customer | after execution; "You will receive this event for internal account transfers (Shaype to Shaype). This event is for the sender of the funds." [docs:payments]; a reversal carries `returnReason` | [docs:payments] (NB: the page's INTRABANK_TRANSFER_OUT samples actually show `"transactionType": "INTERBANK_TRANSFER_OUT"` — §6 Q7) |
| `INTERBANK_TRANSFER_IN` | `false` | ext: inbound NPP / DE credit; mock:`generateInboundNppTransaction`, mock:`generateInboundNppTransactionV2` (also the settlement leg of a PayTo payment when the client is creditor), mock:`generateInboundDeTransaction` with `transactionType: CREDIT` `[inferred for the DE mock]` | on receipt of funds from an external bank; "After the transfer execution, both the sender and receiver will receive a webhook notification" [docs:direct-debits] (Direct Credit section) | [docs:payments] [docs:direct-debits] [docs:payto-staging-testing-suite] |
| `INTERBANK_TRANSFER_OUT` | `false` | op:`makeTransferV1` to a non-Shaype BSB (NPP if eligible else DE); batch items from `createBatch` (`ACCOUNT_TRANSFER`) `[spec:batch]`; scheduled payments executing (`originType: SCHEDULED_PAYMENT`) `[inferred]` | after execution; "After the transfer execution, both the sender and receiver will receive a webhook notification" [docs:direct-debits] (Direct Credit section); NPP return → second event with positive amount and `returnReason` (e.g. `CUSTOMER_REQUEST`) | [docs:payments] [docs:direct-debits] [docs:batch-api] |
...
| `CUSTOMER_STATUS_UPDATED` | op:`changeHayCustomerStatus` ("also sends a webhook event with the type CUSTOMER_STATUS_UPDATED"); op:`blockCustomer` → `BLOCKED`; op:`unblockCustomer` → `ACTIVE`; platform: account closure making the customer `INACTIVE` `[inferred]` — NB `[docs:account-closure]` lists only Account Status Change and Card Status Change as closure notifications and says future notifications are cancelled when the customer becomes Inactive; a `CUSTOMER_STATUS_UPDATED`→`INACTIVE` webhook may therefore not be sent — make it a mock option (default off); platform: onboarding outcome → `ACTIVE`/`REFERRED`/`REJECTED` `[inferred]` | on status change; `actionOwner: CLIENT` in the sample | [docs:customer-creation-1] [docs:account-closure] |
...
| `REMINDER` (Apple/Google `reminderType`s) | platform: reminder scheduler, started "when a customer starts the process of adding their card to a digital wallet" (or, for the `_ADD_TO_WALLET_REMINDER_30/60/90_DAYS`, when the card is created but provisioning never started) | 24h / 7d / 14d / 30d / 60d / 90d schedules as named; `APPLE_PAY_REMINDER_7_DAYS` and `GOOGLE_PAY_7_DAYS_PARTIAL_PROVISIONING`: "Only triggered on first attempt of partial provisioning attempt" | [docs:apple-and-google-pay-notifications] |
...
| `MANDATE_PAYMENT` | op:`makeAdhocPayment` (staging: "will receive a RJCT PSR notification" `[spec:b2b makeAdhocPayment description]` — "PSR" appears on no docs page); platform: scheduled mandate payments; mock:`generateReceiveAPaymentInstruction` (`transactionStatus` `ACCP`/`RJCT`) `[inferred]` | "sent for all payment instructions when the final status is known"; `paymentStatus` + `isFinal` + `reasonCode` on rejection | [docs:payto-notifications] [docs:payto-payment] [spec:b2b] |
...
| Q7 | `[docs:payments]` INTRABANK_TRANSFER_OUT section: its two samples carry `"transactionType": "INTERBANK_TRANSFER_OUT"` and a BSB counterpart (the "Original"/"Return" pair) — copy-paste error, or does an internal transfer to another Shaype account really surface as INTERBANK? | docs inconsistency | Emit `INTRABANK_TRANSFER_OUT`/`_IN` for Shaype-to-Shaype; `INTERBANK_*` for external. |
...
| Q11 | Is there a client-response **timeout** for webhook delivery? (Ext-auth documents 1.2 s / 10 s per transaction type; nothing is documented for webhooks.) | none | Mock: configurable, default 10 s. |
...
| Q14 | Who receives `CUSTOMER_STATUS_UPDATED` when onboarding resolves (`PENDING_APPROVAL → ACTIVE/REFERRED/REJECTED`) — is it emitted in addition to `ONBOARDING_PASSED`/`ONBOARDING_FAILED`? And is it emitted at all when account closure moves the customer to `INACTIVE`? `[docs:account-closure]` lists only Account Status Change and Card Status Change as closure notifications and says future notifications are cancelled when the customer becomes Inactive, so a `CUSTOMER_STATUS_UPDATED`→`INACTIVE` webhook may not be sent. | none for onboarding; [docs:account-closure] cuts against the closure case | Mock: emit both for onboarding `[inferred]`; closure→`INACTIVE` webhook as an option, default off. |
