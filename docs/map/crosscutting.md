...
6. **Pagination limits** — max `limit` for the legacy flavour is only stated (1000) on the direct-debit ops; "offset must be a multiple of limit" appears only there; default ordering of unsorted lists and sort direction of `searchTransactions.sortBy` are unstated.
