...
- Webhooks: `ACCOUNT_STATUS_CHANGE` ("Account Status Change") — listed under the docs' "Synchronous Outcomes" section but the callout attributes it to asynchronous processing ("Based on the asynchronous processing you will receive the following Notification events") [docs:account-closure]; emission timing [open]. `CARD_STATUS_CHANGE` ("Card Status Change") from asynchronous card voiding [docs:account-closure]. `CUSTOMER_STATUS_UPDATED` with `customerStatus: INACTIVE` when the customer is deactivated [inferred from webhook-spec enum; docs say the customer status changes but do not name the event].
...
- Purpose: "Get all Authorisation Holds by Account ID" — "retrieve the authorisation hold details by account Id" [docs:accounts-overview]; these are card transactions "authorised but not yet cleared" [spec HayAccount.heldBalance] / "authorised card payments that have not yet cleared" [docs:account-balances]. Not deprecated.
- Path params: `accountId` (uuid, required).
- Request body: none.
...
- Request body (required): `CreateAccountRequestBody` — required: `accountHolderId`, `accountHolderType`, `idempotencyKey`, `productId` [spec].
  ...
  - `fx` — `AccountFxDataRequest` ("FX-specific data passed through when creating an account"), optional:
    ...
    - `compliance` — `FxComplianceDataRequest` ("FX compliance data passed through when creating an account"), all optional; all `nullable` except `expectedTransactionCountries` [spec]:
...
- Behaviour:
  ...
  - Breach outcomes (transaction side, not this endpoint): the docs example (an ATM withdrawal) shows `outcome: LIMIT_BREACH`, `detailedOutcome: REFUSED_DAILY_ATM_WITHDRAWAL_LIMIT_BREACHED` [docs:account-limits]; the B2B `TransactionOutcome` enum value is `REFUSED_LIMIT_BREACH` [spec]; full outcome list in section 6.
- Webhooks: none.
...
- Behaviour:
  ...
  - `ownerId` is the account holder's customer ID for account-level rules, or a client reference for product-wide rules ("either the Customer ID or Client Reference if a rule applied across the product") [spec]; that product-wide rules are created outside this API is [inferred].
...
- Behaviour: marks the rule `disabled: true` [inferred from naming]; a disabled rule no longer blocks transactions [inferred from the operation name and the `disabled` field]. The disable capability itself is documented ("define the transaction blocking rules, disable rules and retrieve the rule details") [docs:account-rules]. Whether `getAccountRuleById` still returns it afterwards — [open]. Rule belonging to a different account → 4xx [inferred]. Re-disabling — [inferred] `success: true`.
- Webhooks: none.
...
- Request body (required): `TransferOutRequestBody` — required: `amount`, `description`, `senderCustomerHayId`, `transferType` [spec]. **`idempotencyKey` is NOT in the required list** [spec].
  ...
  - `category` — string, optional, "Used to assign a category of the transfer"; free-form string, no enum [spec]. Example values seen: `EATING_OUT` [spec 200 example]; `SAVING`, `BANK_TRANSFER`, `"Shaype POC"` [docs:payments]; `SHOPPING` [docs:account-rules].
...
- Behaviour [docs:payments unless noted]:
  ...
  - Preconditions on the sender account [spec outcomes + docs:account-status/payment-transaction-outcome]: status `LOCKED` → `REFUSED_ACCOUNT_BLOCKED`; `CLOSED` → `REFUSED_ACCOUNT_CLOSED`; `availableBalance < amount` → `REFUSED_INSUFFICIENT_FUNDS` [inferred — spec enum value chosen by name; docs:payment-transaction-outcome describes `REFUSED_NOT_ENOUGH_FUNDS`, which is webhook-enum-only] (stacks never draw down [docs:stack]); risk level `HIGH` → all limits 0 → limit refusal [docs:account-limits]; `PAYMENT_TO_ACCOUNT_NUMBER` (per-transfer max) and `TOTAL_SPEND_PER_YEAR` → `REFUSED_LIMIT_BREACH` [inferred — spec enum value chosen by name; docs describe `REFUSED_ANNUAL_SPENDING_LIMIT_BREACHED`, webhook-enum-only]; daily transfers-out limit → `REFUSED_DAILY_TRANSFERS_OUT_LIMIT_BREACHED` [docs:payment-transaction-outcome]. Only `REFUSED_ACCOUNT_BLOCKED` / `REFUSED_ACCOUNT_CLOSED` / `REFUSED_DAILY_TRANSFERS_OUT_LIMIT_BREACHED` in this bullet are docs-sourced.
...
- Webhooks [docs:payments]: `TRANSACTION` events with `transactionEvent.transactionType` `INTRABANK_TRANSFER_OUT` (sender, internal), `INTRABANK_TRANSFER_IN` (recipient, internal), `INTERBANK_TRANSFER_OUT` (sender, external), `INTERBANK_TRANSFER_IN` (external inbound). Reversal webhooks include `returnReason`. `transactionEvent.accountBalances` = `{ totalBalance, heldBalance, lockedBalance, stacksBalance, availableBalance }` each a `CurrencyAmount`, plus `updatedBalance` [webhook-spec AccountBalancesDto; docs:payments sample]. Caveat: the docs:payments samples also include `accountBalances.legacyAvailableBalance` (not in webhook-spec `AccountBalancesDto` — do not rely on it) and the samples under the INTRABANK_TRANSFER_OUT heading actually carry `transactionType: INTERBANK_TRANSFER_OUT`; the webhook-spec enum is authoritative. `ACCOUNT_STATUS_CHANGE` (`APPROVED → ACTIVE`) on first transaction [inferred].

## 2. Entities and fields

### HayAccount ("Details of an account") [spec]

`required: ["customData"]` — the only required property (odd but verbatim). All numbers are "to 2 decimal places". Example values from [docs:sample-requests-responses] create-account response, except `customData` (that sample has no `customData`; its example is from the spec's createAccount request example "Create group account with specific number and custom data").
...
| `customData` | object | yes | required key | Contains custom metadata stored with the Account | `{"key":"value"}` [spec request example, not the docs sample] |
...
| `ACTIVE` [inferred] | `DORMANT` | platform inactivity timer ("for a specific period of time") — no API, period unspecified; the spec description does not name the source status | [spec enum description for the target; from-state inferred] |
...
| from | to | via |
|---|---|---|
| `ACTIVE` | `BLOCKED` | `blockAccount` with `accountBlockStyle` absent or `ACCOUNT_AND_CUSTOMER` [spec: customer is blocked; inferred: status value `BLOCKED`] |
...
- Limit breach on an ATM withdrawal (docs example) surfaces as `outcome: LIMIT_BREACH` / `detailedOutcome: REFUSED_DAILY_ATM_WITHDRAWAL_LIMIT_BREACHED` in the docs example [docs:account-limits] (field names as shown there; the B2B `TransactionOutcome` only has `outcome`).
...
- `reason` (risk level) 1–128 chars; `note` (block/unblock) ≥ 1 char; transfer `description` 1–255; `reference` 0–35; `recipientName` 1–140; `senderName` 1–140 for INTERNAL, 0–140 for ACCOUNT/PAY_ID [spec].
...
- **Customers**: `createAccount` requires the holder customer to exist and be `ACTIVE` [docs:customer-status-flow]. `makeTransferV1.senderCustomerHayId` must be a customer (holder or group member) [inferred]. Customer `status` enum (spec `HayCustomer`, spec order): `ACTIVE`, `INACTIVE`, `REJECTED`, `BLOCKED`, `PENDING_APPROVAL`, `REFERRED` [spec].
...
HTTP-level (every operation) [spec]: `400 Bad Request`, `403 Forbidden`, `422 Unprocessable Content` (closeAccount says "Unprocessable Entity"), `500 Internal Server Error`, `501 Not Implemented` → `ErrorResponse { details, message, status, traceId }` — except blockAccount (422 "One or more accounts in scope could not be blocked" → `BlockAccountResponse`) and closeAccount (422 "Unprocessable Entity" → `CloseAccountResponse`) [spec]. No message texts are given for these in the spec.
...
Transaction outcomes: `TransactionOutcome.outcome` (makeTransferV0/V1, 21 values [spec], listed verbatim under makeTransferV1) is a subset; the full list below is the webhook `transactionEvent.outcome` enum (41 values [webhook-spec]) with meanings from [docs:payment-transaction-outcome]. Values marked **(webhook-only)** are not in the B2B `TransactionOutcome` enum and cannot be returned by makeTransferV0/V1; the 16 values in both enums are `ACCEPTED`, `INTERNAL_ERROR`, `REFUSED_FRAUD`, `REFUSED_ACCOUNT_BLOCKED`, `REFUSED_RECIPIENT_ACCOUNT_BLOCKED`, `REFUSED_ACCOUNT_CLOSED`, `REFUSED_RECIPIENT_ACCOUNT_CLOSED`, `REFUSED_DAILY_TRANSFERS_OUT_LIMIT_BREACHED`, `REFUSED_MAX_BALANCE_EXCEEDED`, `REFUSED_TOTAL_INBOUND_DIRECT_DEBIT_DAILY_LIMIT_BREACHED`, `REFUSED_TOTAL_OUTBOUND_BPAY_DAILY_LIMIT_BREACHED`, `REFUSED_TOTAL_NET_VISA_DAILY_LIMIT_BREACHED`, `REFUSED_TOTAL_NON_SCHEME_DAILY_LIMIT_BREACHED`, `REFUSED_SENDER_ACCOUNT_NOT_VERIFIED`, `REFUSED_CAPABILITY_NOT_ENABLED`, `REFUSED_QUOTE_EXPIRED` [spec ∩ webhook-spec]:
`ACCEPTED` accepted for processing; `REFUSED_CARD_PREFERENCE` (webhook-only); `REFUSED_FRAUD`; `REFUSED_MAX_BALANCE_EXCEEDED` would exceed `MAX_BALANCE`; `REFUSED_NOT_ENOUGH_FUNDS` would breach `MIN_BALANCE` (webhook-only); `INTERNAL_ERROR`; `REFUSED_ANNUAL_SPENDING_LIMIT_BREACHED` (webhook-only); `REFUSED_DAILY_ATM_WITHDRAWAL_LIMIT_BREACHED` (webhook-only); `REFUSED_DAILY_TOP_UP_LIMIT_BREACHED` (webhook-only); `REFUSED_ACCOUNT_BLOCKED` account is blocked; `REFUSED_ACCOUNT_CLOSED`; `REFUSED_RECIPIENT_ACCOUNT_BLOCKED` (Shaype-to-Shaype); `REFUSED_RECIPIENT_ACCOUNT_CLOSED`; `REFUSED_DAILY_DIRECT_DEBIT_LIMIT_BREACHED` (webhook-only); `REFUSED_DAILY_TRANSFERS_OUT_LIMIT_BREACHED`; `REFUSED_RULES` account rule matched (webhook-only); `REFUSED_TOTAL_INBOUND_DIRECT_DEBIT_DAILY_LIMIT_BREACHED`; `REFUSED_TOTAL_OUTBOUND_BPAY_DAILY_LIMIT_BREACHED`; `REFUSED_TOTAL_NET_VISA_DAILY_LIMIT_BREACHED` "client scheme transactions are currently blocked"; `REFUSED_TOTAL_NON_SCHEME_DAILY_LIMIT_BREACHED`; `REFUSED_BPAY_INVALID_BILLER_CODE` (webhook-only); `REFUSED_BPAY_INVALID_REFERENCE` (webhook-only); `REFUSED_BPAY_INVALID_PAYMENT` (webhook-only); `REFUSED_BPAY_REJECTED` (webhook-only); `REFUSED_DAILY_CARD_TRANSACTIONS_LIMIT_BREACHED` (webhook-only); `REFUSED_SINGLE_CARD_TRANSACTION_LIMIT_BREACHED` (webhook-only); `REFUSED_SANCTIONS` (webhook-only); `REFUSED_UNABLE_TO_VALIDATE` (webhook-only); `REFUSED_INSUFFICIENT_DATA` (webhook-only). Not currently in use (all webhook-only): `REFUSED_ACCOUNT_PREFERENCE`, `REFUSED_DAILY_LIMIT_EXCEEDED`, `REFUSED_AML`, `REFUSED_ACCOUNT_NOT_FOUND_FOR_CARD_TOKEN`, `REFUSED_UNDETERMINED_BALANCE_FOR_ACCOUNT`, `REFUSED_ACCOUNT_NOT_FOUND_FOR_CURRENCY`, `REFUSED_UNDETERMINED_SPENDING_FOR_ACCOUNT`, `REFUSED_UNDETERMINED_TOP_UPS_FOR_ACCOUNT`, `REFUSED_UNDETERMINED_ATM_WITHDRAWALS_FOR_ACCOUNT`. B2B-spec-only values (in `TransactionOutcome` but not the webhook enum) with no docs entry: `REFUSED_LIMIT_BREACH`, `REFUSED_CUSTOMER_PREFERENCE`, `REFUSED_INSUFFICIENT_FUNDS`, `REFUSED_INVALID_PAY_ID`, `UNKNOWN` [spec]. In both enums but with no docs entry: `REFUSED_SENDER_ACCOUNT_NOT_VERIFIED`, `REFUSED_CAPABILITY_NOT_ENABLED`, `REFUSED_QUOTE_EXPIRED` [spec, webhook-spec].
...
28. **Legacy v0 create endpoints (not marked deprecated in the spec)** (`POST /v0/customers/{id}/account`, `POST /v0/groups/{id}/account`) still in the spec — implement as aliases of `createAccount` or omit.
