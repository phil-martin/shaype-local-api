...
- `[docs:<slug>]` — `developer.shaype.com/docs/<slug>.md`. Slugs read: `payto`, `status-transitions`, `payto-operations`, `payto-payment`, `payto-notifications`, `payto-staging-testing-suite`, plus `webhook-notification` (no PayTo content) and `llms.txt` (index; no further PayTo docs pages exist except `/page/payto-staging-testing-suite.md`, a formatting-only duplicate of the docs page — reference links rendered as `METHOD /path`, one `{mandate_id}` placeholder shown as a concrete example ID — with no additional content).
...
- Request body (required): `CreateMandateRequestBody` — "Body of a request to add mandate." [spec]
  ...
  | `partyReference` | string | no | `pattern: ^[ -~]{1,35}$` | "Reference associated with the mandate as provided by party. Specifies a character string with a maximum length of 35 ASCII printable characters." `endToEndId` of scheduled payments is "automatically filled ... by information stored within partyReference provided when mandate is created" [docs:payto-staging-testing-suite]; the docs do not say whether the creditor's or the debtor's `partyReference` is used (`[inferred]` creditor). |
  ...
  | `partyReference` | string | no | `pattern: ^[ -~]{1,35}$` | as creditor. The same `endToEndId` note applies: docs attribute scheduled-payment `endToEndId` to "partyReference provided when mandate is created" without saying which party's [docs:payto-staging-testing-suite]. |
  | `partyType` | string | no | enum `["ORGANISATION","PERSON"]` | |
  ...
  `CurrencyAmount` — `required: ["amount","currency"]`; `amount`: number "Amount of the transaction to 2 decimal places"; `currency`: string, enum of 162 ISO-4217 codes including `"AUD"` [spec]. Every documented example uses `"AUD"` [docs:payto-staging-testing-suite].
...
- Webhooks: `MANDATE` events with triggers `MCRC` (create confirmed), `MCRD` (create declined), `MCRR` (create recalled) arrive later to the Initiator; `MCRX` (create expired) goes to Payer and Initiator [docs:payto-notifications]; `MCRT` ("Mandate Requires Authorisation") to the Payer [docs:payto-notifications]. `MANDATE_ACTION_EXPIRATION` exists in the webhook spec (see §2) but the docs do not tie it to an operation.
...
- Webhooks: `MAMN` ("Mandate Amended") is documented only for the reverse direction (Payer amends → Initiator receives) [docs:payto-notifications]; the Payer-side mock trigger list includes `MAMN` [spec: `GeneratePayerMandateNotificationRequestBody.trigger`]; `[inferred]` the Payer receives `MAMN` for an Initiator unilateral amendment.
...
- Request body (required): `AmendMandatePaymentTermsRequestBody` — "Body of a request to amend mandate payment terms." No `required` list [spec]:
  ...
  | `resolutionRequestedBy` | string | no | "Date time in UTC format yyyy-MM-dd'T'hh:mm:ss.sss'Z' e.g. 2023-09-10T10:00:00.000Z (Optional)." Ignored unless the amendment is bilateral; for bilateral amendments it is included in the notification sent to the counterparty; informational only, does not affect MMS expiry [spec: `GetMandateActionsDetailsAmendmentDto.resolutionRequestedBy`: "It will be ignored if the resulting amendment action is not bilateral. For bilateral amendments, it will be provided in any notification sent to the counterparty. This time is for informational purposes only and does not affect the expiry time imposed by the MMS."]. |
  | `validityEndDate` | string (date) | no | "The date when the mandate stops being valid." |

- Response `200 Success`: `GenericMessage`. Errors: standard.
- Behaviour:
  - **Bilateral** amendment requiring Payer authorisation: the spec's `resolveMandateByPayer` is "Authorise an amend that has been requested by an Initiator" and `resolveMandateByInitiator` "Recalls a new mandate or mandate amendment that has not yet been authorised by the Payer" [spec]; docs: "Amend PayTo agreements (including obtaining authorisation from the Debtor if necessary)" [docs:payto]; "For a bilateral amend action..." [docs:payto-notifications]. Creates an action `type: AMEND`, `bilateral: true`, `status: PENDING`, with `expiryTime` [inferred from action schema]. Payer `ACCEPT` → `MAMC` to Initiator; `REJECT` → `MAMD` to Initiator; no answer → "status of the action is set to Timed Out (TIMO)" (`TIMED_OUT`), `MAMX` to both [docs:payto-notifications]. Action outcome `ACCEPT` → `COMPLETED` (terms applied), `REJECT` → `DECLINED` [spec: `GetMandateActionsActionDto.status` descriptions; inferred].
...
- Webhooks: triggers `MCRR` (create recalled) / `MAMR` (amend recalled) [docs:payto-staging-testing-suite "Depending on which bilateral action is recalled, specific party should receive a notification ... mandate creation (trigger: MCRR) and bilateral amendment (trigger: MAMR)"]. Recipient: docs:payto-notifications lists MCRR → Initiator; MAMR is absent from that table, so its recipient is not documented. `[inferred]` the counterparty (Payer) is also notified — see §7 open question 19.
...
- Behaviour [docs:payto-payment]:
  ...
  - Synchronous wait: in the staging timeout scenario the response arrives "after a 15 second timeout" as `REJECTED`, `statusIsFinal: true` with underlying reason `RJCT AB01` ("Clearing process aborted due to timeout") [docs:payto-staging-testing-suite]; `[inferred]` the synchronous wait is capped at 15 s in all cases (the docs only describe the timeout scenario).
...
`CurrencyAmount` — `required: ["amount","currency"]`; `amount` number "to 2 decimal places"; `currency` 162-value ISO-4217 enum. `CurrencyAmountDto` (MMS/actions variant) — `amount` **string** `^(?=.{1,19}$)[0-9]{0,18}(?:\.[0-9]{0,2})?$`, `currency` string `^[A-Z]{3}$` [spec].
...
`GetMandateActionsDetailsAmendmentDto` — "Request to amend the details of a mandate." No required fields; same field set as Creation minus `establishmentScheme` / `initiationRequestIdentification` / `mandatePurposeCode` / `mandateType` / `validityStartDate`, plus `mandateDetailsCxExtension` (so there is no `details.amendment.mandatePurposeCode`); `bescUserIdentification` is an unconstrained string on the amendment DTO (the 0–6 length constraint is on the creation DTO only) [spec]; **clear-sentinel semantics**: `description`/`shortDescription`/`nppBackofficeService`/`bescUserIdentification` set to `'-'` mean cleared; `validityEndDate` `'1000-01-01'` means cleared [spec]. `resolutionRequestedBy`: "It will be ignored if the resulting amendment action is not bilateral. For bilateral amendments, it will be provided in any notification sent to the counterparty." — informational only, does not affect MMS expiry [spec: `GetMandateActionsDetailsAmendmentDto.resolutionRequestedBy`].
...
| `accountId` | uuid | "Platform identifier of the creditor account" (the Creation and Porting debtor DTOs carry the same copy-pasted "creditor" text; the Amendment debtor DTO says "Platform identifier of the debtor account") [spec] |
...
`GetMandateActionsDetails{Creation,Amendment,Porting}PaymentInitiatorInformationDto`: `partyIdentification` (`^[ -~]{1,35}$`), `partyIdentificationTypeCode` (same 20-value enum), `partyLegalName` (`^[ -~]{1,140}$`), `partyName`; `partyServicerBic` (BIC pattern) is present on the Creation and Porting variants only, not on the Amendment variant [spec]. Creation variant `required: ["partyIdentification","partyIdentificationTypeCode","partyLegalName","partyName"]`.
...
- `GenerateInitiatorMandateNotificationRequestBody.trigger` (4 chars) enum `MCRC,MCRD,MCRX,MAMC,MAMD,MAMN,MAMX,MPOF,MPOT,MPOX,MSCH`; `GeneratePayerMandateNotificationRequestBody.trigger` enum `MCRX,MCRT,MCRP,MAMN,MAMP,MAMR,MAMX,MSCH`; both take `actionDetails {actionId}` and `mandateDetails {mandateId (32-hex), creditorInformation, debtorInformation, description (string 1–140), shortDescription (string 1–35), paymentInformation {amount, countPerPeriod, firstPaymentAmount, firstPaymentDate, lastPaymentAmount, lastPaymentDate, maximumAmount, paymentAmountType (BALN/FIXE/USGB/VARI), paymentFrequency (ADHO/DAIL/FRTN/INDA/MIAN/MNTH/QURT/WEEK/YEAR), pointInTime}, validityStartDate, validityEndDate}` — `GenerateMandateNotificationMandateDetailsDto.required: ["debtorInformation","mandateId","paymentInformation","validityStartDate"]` [spec].
...
| `CREATED` | `CANCELLED` | MMS authorisation timeout after 6 days → `MCRX` to Payer and Initiator [docs:payto-notifications] (cx `CANCELLED_AUTHORISATION_TIMED_OUT`) | [docs:payto-staging-testing-suite "after 6 days mandate should be automatically rejected and set into CANCELED status by MMS"]; cx mapping [inferred] |
...
- **Instruction ID** (`instructionId` / `PaymentInstruction.id`): 35 chars = `<11-char BIC>` + `I` + `YYYYMMDD` + `00` + 12 digits + 1 alphanumeric — pattern `^[A-Z0-9]{4}[A-Z]{2}[A-Z0-9]{2}[A-Z0-9]{3}I[0-9]{8}00[0-9]{12}[0-9a-zA-Z]$` [spec: `PaymentInstructionSummary`]; `ExternalMandatePaymentDetails.instructionId` is 1–35 chars [spec]. Staging examples use BIC `ANNCAU22XXX`, e.g. `ANNCAU22XXXI20230718000000000077240`. Derived NPP ids: `paymentId`/`originalMessageIdentification` = id with the leading `ANNCAU22XXXI2` removed... per docs: "should have the same ID as instructionIdentification but after first part of the ID that indicates Business Identifier Code (BIC - ANNCAU22XXXI2) should be removed"; the `N` insertion is contradictory in the docs: the note says "`OriginalMessageIdentification` should contain additional letter N after BIC code", but the sample request shows `"originalMessageIdentification": "ANNCAU22XXX20230718000000000077240"` (no `N`) and `"transactionIdentification": "ANNCAU22XXXN20230718000000000077240"` (`N` after the BIC) [docs:payto-staging-testing-suite]. `[inferred]` the implementation follows the sample: `transactionIdentification` = BIC + `N` + the 23-digit tail, `originalMessageIdentification` without `N`.
- **endToEndId**: adhoc → request `endToEndId` (1–35 chars); scheduled → `partyReference` supplied at mandate creation (docs do not say whether creditor's or debtor's; `[inferred]` creditor); absent → literal `"Not provided"` [docs:payto-staging-testing-suite].
...
- **Timeouts**: makeAdhocPayment — the staging timeout scenario responds "after a 15 second timeout" [docs:payto-staging-testing-suite], `[inferred]` the synchronous wait is capped at 15 s in all cases; mandate authorisation expiry 6 days [docs:payto-staging-testing-suite]; payment-instruction status retrievable for 15 days [docs:payto-payment].
