 ...
 * - account.statusChanged to CLOSED: every live PayID on the account is deregistered with the closure
 *   reason (docs:account-closure "All PayID's registered to that account are deleted").
 */
...
export function registerEvents(ctx: AppContext, svc: PayIdService): void {
  ...
  ctx.events.on('account.statusChanged', ({ account }) => {
    ...
    svc.deregisterAllForAccount(account.id, account.closeReason)
  })
}
